...
const { bundleRuntime } = require('./bundle-runtime');
...
function code(name,helpers,body,x,y,purpose,inputs,outputs){return add(name,'n8n-nodes-base.code',2,{mode:'runOnceForAllItems',jsCode:bundleRuntime(helpers,body)},x,y,purpose,inputs,outputs);}
function queryField(name,source,x,y,purpose){
const value='={{ (typeof ('+source+') === "string" ? ('+source+').trim().slice(0,400) : "").split(/\\s+/).slice(0,50).join(" ") }}';
return add(name,'n8n-nodes-base.set',3.4,{mode:'manual',assignments:{assignments:[{id:id(name+':active_query'),name:'active_query',type:'string',value}]},includeOtherFields:true,options:{}},x,y,purpose,'Preserved execution state + query','Preserved execution state + active_query');
}
function connectSearch(search,target,input=0){for(const exit of search.exits)edge(exit.node,target,exit.output,input);}
...
const suffix=role[0].toUpperCase()+role.slice(1),prepareHelper='prepare'+suffix,finishHelper='finish'+suffix;
const prep=code('Prepare: '+label,prepareHelper,'return [{json:'+prepareHelper+'($input.first().json, '+JSON.stringify(prompts[role])+', '+JSON.stringify(schemas[role])+')}];',x,y,'Preserve state and compose '+label+' prompt');
...
const parsed=code('Parse: '+label,'readModel','return [{json:readModel($('+JSON.stringify(prep)+').first().json,$input.first().json)}];',x+480,y,'Strict JSON/schema validation; preserve raw response');
...
const capture=code('Capture repair: '+label,'readModel','return [{json:readModel($('+JSON.stringify(parsed)+').first().json,$input.first().json,true)}];',x+1000,y+220,'Retain original raw response if repair also fails');
const end=code('State: '+label,finishHelper,'return [{json:'+finishHelper+'($input.first().json)}];',x+1240,y,'Apply validated role output or safe local fallback');
...
const prep=queryField('Query: '+label,query.replace(/\bs\./g,'$json.'),x,y,'Cap You.com query at 400 characters / 50 words');
...
const collect=code('Evidence: '+label,'captureSearch','const s=$('+JSON.stringify(prep)+').first().json;return [{json:captureSearch(s,$input.all().map(i=>i.json),'+JSON.stringify(dimension)+',s.active_query,new Date().toISOString())}];',x+440,y,'Normalize MCP evidence, preserving source URL/snippet/date');
...
const retryPrep=discovery?queryField('Retry query: '+label,'$json.target_company+" alternatives "+$json.target_product+" "+$json.geography',x+660,y+200,'Rewrite discovery query once'):null;
const second=tool('You.com: '+label+' - transport retry',x+880,y+200);
const retryResult=code('Retry evidence: '+label,'captureSearch','const s=$('+JSON.stringify(retryPrep||collect)+').first().json;return [{json:captureSearch(s,$input.all().map(i=>i.json),'+JSON.stringify(dimension)+',s.active_query,new Date().toISOString(),1)}];',x+1100,y+200,'Capture terminal retry result; no retry back edge');

edge(prep,first);edge(first,collect);edge(collect,retry);edge(retry,retryPrep||second);if(retryPrep)edge(retryPrep,second);edge(second,retryResult);
return {start:prep,exits:[{node:retry,output:1},{node:retryResult,output:0}]};
}
...
const norm=code('N02 Normalize Input','normalize','return [{json:normalize($input.first().json,new Date().toISOString())}];',240,0,'Clean request; initialize bounded execution state');
...
const discoveryEvidence=code('N04 Preserve Discovery Evidence','attachDiscovery','return [{json:attachDiscovery($input.first().json)}];',3760,0,'Retain discovery sources separately');
...
const init=code('N06 Initialize Competitor','initializeCompetitor','return [{json:initializeCompetitor($input.first().json)}];',0,950,'Reset evidence retry budget for each competitor');
...
const evidence=code('N09 Combine Research Evidence','mergeEvidence','return [{json:mergeEvidence($input.all().map(i=>i.json))}];',3700,1450,'Combine sources; reject mixed competitor identity');
...
const inc=code('N13 Increment Evidence Retry','incrementRetry','return [{json:incrementRetry($input.first().json)}];',7200,1760,'Consume budget before search');
const retrySearch=search('Targeted Retry','retry','s.retry_query',7480,1760);
const append=code('N14 Append Retry Evidence','attachRetry','return [{json:attachRetry($input.first().json)}];',9050,1760,'Append evidence and repeat extraction/audit once');
const record=code('N15 Validated Competitor Record','validatedRecord','return [{json:validatedRecord($input.first().json)}];',7200,1450,'Withhold unsupported/conflicting fields; retain gaps and audit evidence','Profile/audit/evidence','Safe competitor record');
...
const scope=code('N16 Restore Research Scope','aggregate','return [{json:aggregate($("State: Discovery").first().json,$input.first().json.records)}];',6560,-450,'Restore scope and discovery gaps');
const empty=code('N16 No Verified Competitors','aggregate','return [{json:aggregate($input.first().json,[])}];',5980,-700,'Avoid dead end on empty competitor split');
const synthesis=stage('Synthesis','synthesis',0,3600);
const review=code('N18 Prepare Approval','approvalView','return [{json:approvalView($input.first().json)}];',1550,3600,'Preserve latest draft and escape HTML');
...
const decision=code('N18 Capture Human Decision','approval','return [{json:approval($("N18 Prepare Approval").first().json,$input.first().json)}];',2050,3600,'Restore trusted latest state after form replaces input');
...
const revInc=code('N20 Increment Revision Count','beginRevision','return [{json:beginRevision($input.first().json)}];',2800,3900,'Increment revision count before model call');
const revision=stage('Revision','revision',3050,3900);
const output=code('N21 Approved Markdown Output','approvedOutput','return [{json:approvedOutput($input.first().json)}];',2550,3600,'Guard approval; emit final report_markdown','Approved draft','report_markdown + audit state');
...
edge(plan.end,discoverySearch.start);connectSearch(discoverySearch,discoveryEvidence);edge(discoveryEvidence,discovery.start);edge(discovery.end,have);
edge(have,split);edge(have,empty,1);edge(split,loop);edge(loop,agg,0);edge(loop,init,1);edge(init,queries.start);
branches.forEach((b,i)=>{edge(queries.end,b.start);connectSearch(b,merge,i);});
edge(merge,evidence);edge(evidence,extractor.start);edge(extractor.end,validator.start);edge(validator.end,retry);
edge(retry,inc);edge(retry,record,1);edge(inc,retrySearch.start);connectSearch(retrySearch,append);edge(append,extractor.start);edge(record,loop);
...
const workflow={name:'Multi-Agent Competitor Research - You.com + Gemini',nodes,connections,active:false,settings:{executionOrder:'v1',saveManualExecutions:true,saveDataErrorExecution:'all',saveDataSuccessExecution:'all'},tags:[]};
const exported=JSON.stringify(workflow,null,2)+'\n';
const bytes=Buffer.byteLength(exported,'utf8');
if(bytes>=500000)throw new Error('Workflow size budget exceeded: '+bytes+' bytes (must remain below 500,000)');
fs.writeFileSync(path.join(root,'workflow/competitor-research-agent.json'),exported);
...
console.log('Built '+nodes.length+' nodes, '+bytes+' bytes.');
